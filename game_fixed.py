...
try:
    ...
    print(f"Проверьте, что все изображения находятся в правильных папках:")
    print(f"- {IMAGE_DIR}")
    sys.exit(1)
...
class Player:
    def __init__(self, x, y):
        ...
        try:
            ...
            for i in range(1, 9):
                img = pygame.image.load(os.path.join(IMAGE_DIR, 'player_right', f'player_right{i}.png'))
                img = pygame.transform.scale(img, (PLAYER_WIDTH, PLAYER_HEIGHT))
                self.animations['right'].append(img)
                
                img = pygame.image.load(os.path.join(IMAGE_DIR, 'player_left', f'player_left{i}.png'))
                img = pygame.transform.scale(img, (PLAYER_WIDTH, PLAYER_HEIGHT))
                self.animations['left'].append(img)
            
            # Load jump animations
            for i in range(1, 10):
                img = pygame.image.load(os.path.join(IMAGE_DIR, 'player_jump', 'player_jump_right', f'Jump_right{i}.png'))
                img = pygame.transform.scale(img, (PLAYER_WIDTH, PLAYER_HEIGHT))
                self.animations['jump_right'].append(img)
                
                img = pygame.image.load(os.path.join(IMAGE_DIR, 'player_jump', 'player_jump_left', f'Jump_left{i}.png'))
                img = pygame.transform.scale(img, (PLAYER_WIDTH, PLAYER_HEIGHT))
                self.animations['jump_left'].append(img)
                
            # Load attack animations
            for i in range(1, 5):
                img = pygame.image.load(os.path.join(IMAGE_DIR, 'player_Attack', 'Attak_right', f'Attack{i}right.png'))
                img = pygame.transform.scale(img, (PLAYER_WIDTH * 1.5, PLAYER_HEIGHT))
                self.animations['attack_right'].append(img)
                
                img = pygame.image.load(os.path.join(IMAGE_DIR, 'player_Attack', 'Attak_left', f'Attack{i}left.png'))
                img = pygame.transform.scale(img, (PLAYER_WIDTH * 1.5, PLAYER_HEIGHT))
                self.animations['attack_left'].append(img)
                
            # Load death animations
            for i in range(1, 7):
                img = pygame.image.load(os.path.join(IMAGE_DIR, 'player_dead', f'Dead{i}.png'))
                img = pygame.transform.scale(img, (PLAYER_WIDTH, PLAYER_HEIGHT))
                self.animations['dead'].append(img)
        except pygame.error as e:
            print(f"Ошибка при загрузке анимаций персонажа: {e}")
            sys.exit(1)
    ...
    def draw(self, screen):
        ...
        # Calculate draw position for attack animations
        if self.is_attacking:
            # When attacking, the sprite is wider, so adjust x position
            x = self.x - (PLAYER_WIDTH * 0.25) if self.facing_right else self.x
            screen.blit(self.current_image, (x, self.y))
        else:
...
class Platform:
    def __init__(self, x, y, width, texture_index=0):
        ...
        self.texture = building_textures[texture_index % len(building_textures)]
...
# Create game objects
player = Player(WIDTH // 2, HEIGHT - PLAYER_HEIGHT - 50)
platforms = [
